...
def load_data(path: str) -> pd.DataFrame:
    df = pd.read_csv(
        path,
        dtype={k: v for k, v in DTYPE_MAP.items()},   # only apply known cols
        parse_dates=["acq_date"],
        infer_datetime_format=True,
    )
    # Ensure acq_date is datetime even if parse_dates was silently skipped
...
with col1:
    ...
    monthly = (
        ...
        .resample("ME")          # "ME" = month-end; avoids FutureWarning in Pandas 2.2+
        .size()
