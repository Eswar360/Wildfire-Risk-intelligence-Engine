...
@st.cache_data
def load_data(file):
    df = pl.read_csv(file, infer_schema_length=5000)
    # Parse date
    df = df.with_columns(pl.col("acq_date").str.to_date("%Y-%m-%d"))
    return df
...
pdf = df.to_pandas()
pdf["acq_date"] = pd.to_datetime(pdf["acq_date"])
...
selected_conf = st.sidebar.multiselect(
    ...
    format_func=lambda x: conf_map.get(x, x)
)
...
with col1:
    ...
    monthly = filtered.set_index("acq_date").resample("ME").size().reset_index()
...
with col2:
    ...
    ax.axvline(filtered["brightness"].mean(), color="gold", linestyle="--", linewidth=2, label=f"Mean: {filtered['brightness'].mean():.1f} K")
